# IoT Data ETL Pipeline using Apache Spark

## Overview
This project implements a scalable ETL pipeline using Apache Spark on Databricks to process large-scale IoT sensor data.

## Features
- Synthetic dataset generation (~1GB)
- Data cleaning and transformation
- Aggregation and analytics
- Anomaly detection
- Distributed processing using Spark

## Tech Stack
- Apache Spark (PySpark)
- Databricks
- Python

## Results
- Average sensor values by type
- Peak activity location
- Detection of abnormal sensor readings

## How to Run
1. Import notebook into Databricks
2. Run cells sequentially
3. View outputs using display()

Updated using Git workflow

## Author
Aishwarya R
